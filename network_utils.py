...
def create_loss(scores,
    ...
    if source_adapt_scope is not None:
        source_adapt = tf.reshape(end_points[scope+'/'+source_adapt_scope], [batch_size / 2, -1])
        target_adapt = tf.reshape(end_points[scope+'/'+target_adapt_scope], [batch_size / 2, -1])
        mmd_loss = create_mmd_loss(source_adapt, target_adapt)
    loss = sig_loss if mmd_loss is None else sig_loss + lamb * mmd_loss
    return loss, accuracy


def create_multi_mmd_loss(scores,
                          end_points,
                          class_labels,
                          theta_labels,
                          scope=None,
    ...
    conf = tf.equal(tf.to_int32(tf.greater_equal(sig_op, 0.5)),
                    tf.to_int32(tf.greater_equal(tf.to_float(class_labels), 0.1)))
    accuracy = tf.reduce_mean(tf.to_float(conf))
    source_adapt_1 = tf.reshape(end_points[scope + '/' + source_adapt_scope], [batch_size / 2, -1])
    target_adapt_1 = tf.reshape(end_points[scope + '/' + target_adapt_scope], [batch_size / 2, -1])
    mmd_loss_1 = create_mmd_loss(source_adapt_1, target_adapt_1)
    source_adapt_2 = tf.reshape(end_points[scope + '/fc7' + source_adapt_scope], [batch_size / 2, -1])
    target_adapt_2 = tf.reshape(end_points[scope + '/fc7' + target_adapt_scope], [batch_size / 2, -1])
    mmd_loss_2 = create_mmd_loss(source_adapt_2, target_adapt_2)
    loss = sig_loss + lamb1 * mmd_loss_1 + lamb2 * mmd_loss_2
    return loss, accuracy


def create_semi_supervised_loss(scores,
                                end_points,
                                class_labels,
                                theta_labels,
                                scope=None,
    ...
    source_adapt = tf.reshape(end_points[scope + '/' + source_adapt_scope], [batch_size / 2, -1])
    target_adapt_1 = tf.reshape(end_points[scope + '/' + target_adapt_scope], [batch_size / 2, -1])
    mmd_loss_1 = create_mmd_loss(source_adapt, target_adapt_1)
    target_adapt_2 = tf.reshape(end_points[scope + '/' + target_adapt_scope + '_u'], [batch_size / 2, -1])
    mmd_loss_2 = create_mmd_loss(source_adapt, target_adapt_2)
    loss = sig_loss + lamb1 * mmd_loss_1 + lamb2 * mmd_loss_2
    return loss, accuracy
