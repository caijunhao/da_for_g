...
def batch_norm_scope(batch_norm_decay=0.997,
    ...
    with tf.variable_scope(scope, 'alexnet_v2', [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
            ...
            with slim.arg_scope([slim.conv2d],
                ...
                target_end_points[sc.name + adapt_scope] = net
                net = slim.conv2d(net, num_classes, [1, 1],
                                  activation_fn=None,
                                  normalizer_fn=None,
                                  biases_initializer=tf.zeros_initializer(),
            ...
            end_points.update(target_end_points)
            if spatial_squeeze:
                net = tf.squeeze(net, [1, 2], name='fc8/squeezed')
                end_points[sc.name + '/fc8'] = net
    return net, end_points
    ...
    with tf.variable_scope(scope, 'alexnet_v2', [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
            ...
            with slim.arg_scope([slim.conv2d],
                ...
                channel_size = net.get_shape().as_list()[-1]
                sqeeze = slim.conv2d(net, channel_size / reduction_ratio, [1, 1], scope=adapt_scope + '/squeeze')
                with slim.arg_scope(batch_norm_scope()):
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
            ...
            with slim.arg_scope(batch_norm_scope()):
                with slim.arg_scope([slim.batch_norm], is_training=is_training):
                    ...
                    net = slim.conv2d(net, 64, [3, 3], 2, scope='pool1')
                    net = se_module(net, reduction_ratio=reduction_ratio, scope='pool1')
                    net = slim.conv2d(net, 192, [5, 5], scope='conv2')
                    net = se_module(net, reduction_ratio=reduction_ratio, scope='conv2')
                    net = slim.conv2d(net, 192, [3, 3], 2, scope='pool2')
                    net = se_module(net, reduction_ratio=reduction_ratio, scope='pool2')
                    net = slim.conv2d(net, 384, [3, 3], scope='conv3')
                    ...
                    net = slim.conv2d(net, 256, [3, 3], 2, scope='pool5')
                    net = se_module(net, reduction_ratio=reduction_ratio, scope='pool5')
    ...
    with tf.variable_scope(scope, 'alexnet_v2', [inputs], reuse=reuse) as sc:
        ...
        with slim.arg_scope([slim.conv2d, slim.fully_connected, slim.max_pool2d],
            ...
            with slim.arg_scope([slim.conv2d],
                ...
                net = slim.conv2d(net, 4096, [1, 1], scope='fc7'+adapt_scope)
                net = se_module(net, reduction_ratio=reduction_ratio, scope='fc7'+adapt_scope)
                target_end_points[sc.name + '/fc7' + adapt_scope] = net
