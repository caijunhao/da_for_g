...
def main():
    ...
    with tf.Graph().as_default():
        with tf.device(tf.train.replica_device_setter(args.task_id)):
            ...
            images_p, class_labels_p, theta_labels_p = get_dataset(os.path.join(args.source_dir, 'positive'),
                                                                   args.num_readers,
                                                                   args.num_preprocessing_threads,
                                                                   hparams)
            images_n, class_labels_n, theta_labels_n = get_dataset(os.path.join(args.source_dir, 'negative'),
                                                                   args.num_readers,
                                                                   args.num_preprocessing_threads,
                                                                   hparams)
